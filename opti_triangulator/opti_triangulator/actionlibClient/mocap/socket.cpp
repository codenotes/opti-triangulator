...
#include <sstream>
#include <stdint.h>
typedef uint32_t socklen_t; //greg
...
UdpMulticastSocket::UdpMulticastSocket( const int local_port, const std::string multicast_ip ) 
{
  ...
  int option_value = 1;
//  int result = setsockopt( m_socket, SOL_SOCKET, SO_REUSEADDR, (void*)&option_value, sizeof( int ) ); //greg
  int result = setsockopt(m_socket, SOL_SOCKET, SO_REUSEADDR, (const char*)&option_value, sizeof(int));
  if( result == -1 )
  {
